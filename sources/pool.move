...
module lesson9::flash_lender {
    ...
    const EWrongFlashLenderId: u64 = 1;
    const EWrongRepayAmount: u64 = 2;
    const EAdminOnly: u64 = 3;
    const EWrongWithdrawAmount: u64 = 4;
   ...
   /// Hủy bỏ nếu `amount` lớn hơn số tiền mà `lender` có sẵn để cho vay.
    public fun loan<T>(
        ...
        assert!(amount <= max_loan(self), EAmountTooLarge);
        let flash_lender_id = object::id(self);
        let receipt = Receipt<T> {
            flash_lender_id,
            repay_amount: amount + fee(self)
        };
        let loan_coin = coin::take(&mut self.to_lend, amount, ctx);
        (loan_coin, receipt)
    }
    ...
    public fun repay<T>(self: &mut FlashLender<T>, payment: Coin<T>, receipt: Receipt<T>) {
        ...
        assert!(coin::value(&payment) == repay_amount, EWrongRepayAmount);
    ...
    public fun withdraw<T>(self: &mut FlashLender<T>, admin_cap: &AdminCap, amount: u64, ctx: &mut TxContext): Coin<T> {
        ...
        assert!(amount <= max_loan(self), EWrongWithdrawAmount);
